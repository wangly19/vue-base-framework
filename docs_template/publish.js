...
function buildNav (members) {
  let globalNav
  // let nav = '<h2><a href="index.html">开发文档</a></h2>'
  let nav = ''
  const seen = {}
  const seenTutorials = {}
 ...
 * @return { Array<object> }
 */
function createVuePages (modules) {
  const VueComponentModules = []
  const VuePageModules = []
  const JSModules = []
  modules.forEach(module => {
    const { meta, comment } = module
    const isVueModule = meta.filename.includes('.vue')
    if (isVueModule) {
      const isComponent = comment.indexOf('@component') !== -1
      const isPages = comment.indexOf('@page') !== -1
      isPages && VuePageModules.push(module)
      isComponent && VueComponentModules.push(module)
    } else {
